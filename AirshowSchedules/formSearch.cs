...
namespace AirshowSchedules
{
  internal class formSearch : Form
  {
    ...
    private void InitializeComponent()
    {
      ...
      lstAirshows.Size = new Size(550, 199);
      ...
      lstContacts.Size = new Size(191, 199);
      ...
      // btnGenerateReport
      // 
      btnGenerateReport.Location = new Point(21, 430);
      btnGenerateReport.Name = "btnGenerateReport";
      btnGenerateReport.Size = new Size(155, 23);
      ...
      // btnSaveChanges
      // 
      btnSaveChanges.Location = new Point(343, 430);
      ...
      // button1
      // 
      button1.Location = new Point(182, 430);
      button1.Name = "button1";
      button1.Size = new Size(155, 23);
      button1.TabIndex = 27;
      button1.Text = "Generate Contact List";
      button1.UseVisualStyleBackColor = true;
      button1.Click += btnOutputEmailList_Click;
      // 
      // formSearch
      // 
    ...
    private void btnOutputEmailList_Click(object sender, EventArgs e)
    {
      string result = "";

      result = "Email,Phone,FirstName,LastName,FullName,Airshows\n";
      foreach (cContact contact in officialContacts)
      {
        string FirstName = "";
        string LastName = "";
        List<Airshow> theseShows = Airshow.GetAirshows(contact, officialAirshowGroup.GetAirshows());
        if (contact.name.Length > 0)
        {
          if (contact.name.Contains(' '))
          {
            FirstName = contact.name.Substring(0, contact.name.IndexOf(" "));
            LastName = contact.name.Substring(contact.name.IndexOf(" ") + 1);
          }
          else
          {
        ...
        {
          result += "noemail" + "," + contact.phone + "," + FirstName + "," + LastName + "," + contact.name + "," + "noshows" + "\n";
        }
        else
        {
          ...
          {
            string showsList = "";
            foreach (var show in theseShows)
            {
              showsList += show.name_airshow + "; ";
            }
            result += email + "," + contact.phone + "," + FirstName + "," + LastName + "," + contact.name + "," + showsList + "\n";
          }
        }
      }
      ...
      // write out to a file on the desktop:
      string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
      string filePath = desktopPath + "\\Contacts.csv";
      System.IO.File.WriteAllText(filePath, result);
