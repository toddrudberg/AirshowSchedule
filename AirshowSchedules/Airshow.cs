...
namespace AirshowSchedules
{
  public class Airshow
  {
    ...
    public static List<Airshow> GetAirshows(cContact thisContact, List<Airshow> theseAirshows)
    {
      List<Airshow> result = new List<Airshow>();
      foreach (int showID in thisContact.showIDs)
      {
        Airshow show = theseAirshows.FirstOrDefault(s => s.ID == showID);
        if (show != null)
        {
          result.Add(show);
        }
